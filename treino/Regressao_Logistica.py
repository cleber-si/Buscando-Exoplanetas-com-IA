...
logistc_clf = LogisticRegressionCV(cv=5, random_state = 23)

logistc_clf.fit(X_treino, y_treino)
#y_pred = logistc_clf.pred(X_teste)    # ---> Necessário carregar os dados de teste

# Treina o modelo com os dados aumentados
logistc_clf_A = LogisticRegressionCV(cv=5, random_state = 23)

logistc_clf_A.fit(X_treino_a, y_treino_a)
...
salva_ML_arq = salva_arq + "/logistic_clf.joblib"
dump(logistc_clf, salva_ML_arq)

# Salva o segundo modelo
salva_ML_arq = salva_arq + "/logistic_clf_A.joblib"
dump(logistc_clf_A, salva_ML_arq)
